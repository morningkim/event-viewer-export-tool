...
이 저장소의 작업 지침은 `WORKING_RULES.md`에 정리되어 있습니다. 새 작업을 시작할 때도 그 지침을 먼저 확인합니다.

## 현재 방식

이 도구는 더 이상 `eventvwr.exe`를 자동 실행하지 않습니다.

대신 아래 파일을 생성하고, 화면에 `이벤트 뷰어 열기` 버튼을 띄웁니다.
...
3. 결과 파일 생성
4. 결과 창에 표시된 `Source`, `Event ID`, `Logged` 값을 확인
5. 결과 창에서 `이벤트 뷰어 열기` 버튼 클릭
6. Event Viewer에서 결과 창에 표시된 `Winlogon` 이벤트를 직접 클릭
...
2. 입력 창에서 아래 값을 넣습니다.
   ...
   - `EndTime`
   - `Mode`
3. 결과 창에서 `이벤트 뷰어 열기` 버튼을 클릭합니다.
4. 결과 창에 표시된 `Source`, `Event ID`, `Logged` 값을 확인합니다.
5. Event Viewer에서 같은 값을 가진 이벤트를 클릭합니다.
...
- 사용자가 직접 버튼을 클릭해 Event Viewer를 엽니다.

## 주의 사항

- Event Viewer는 결과 창의 버튼을 사용자가 직접 클릭해서 열어야 합니다.
- `_guide.txt`는 보조 파일입니다. 기본 작업은 결과 창에 표시된 `Source`, `Event ID`, `Logged` 값을 보고 진행합니다.
